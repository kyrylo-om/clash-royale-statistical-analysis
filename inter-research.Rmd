...
dataset to see how different factors impact the outcome of a match. This
dataset offers incredibly broad statistics for each of its 37.9 million
matches, from trophy counts of the players to hit-points of the towers
at the end of the match, but, since we only have two hypotheses to make,
we will mainly focus on two of them: average elixir costs of a deck and
the sum of the deck's card levels. In this research we will study and
test how they influence the outcome of the match, with our results being
applicable to esports, where they can be used to build optimal
strategies and win games more frequently in worldwide tournaments.
...
Before we will try to make hypotheses we need to find general info about our data set. For each card in the 16 (8 per deck) in one match, each has the level and its elixir cost. In our discussions we will be using averages(for elixir) and sums(for levels) for the selected deck . For those we will find the min, max, mean, variance and standard deviation.
...
-   **Null hypothesis** $H_0$: The proportion of wins in
    both groups are equal (the player with the larger card level sum
    has the same chance of winning as the player with the lower sum).

-   **Alternative hypothesis** $H_1$: The number of wins in
    the group where the winning player has a higher card level sum is
    bigger than that of the other group (the player with the larger card
...
We can also compute the p-value, which for a z-test is just:
...
elixir regenerates at set rates throughout the match. Each card has an
elixir cost, which then defines the "average elixir cost" of a deck from
the 8 cards in a deck. If the average is very low the cards are usually
too weak and it’s hard to make an attack, but if it’s too high this can
hinder the deck, as due to elixir having to regenerate, the player has
to wait longer before playing a card and therefore has no options when
they have to defend. So, we will test if it's true that a player has
less chances of winning if their deck has an average elixir cost far
from the universal mean. (E.g. the average cost is too high or too low)
...
-   **Null hypothesis** $H_0$: the slope of the regression equation is
    equal to zero. (Means that the average elixir cost of a deck does
    not affect the probability of winning in any way)\
-   **Alternative hypothesis** $H_1$: the slope of the regression
    equation is negative. (Means that the probability of winning gets
    smaller with greater deviation of the average elixir cost from the
    universal mean)
...
Not the best fit, but it doesn't really matter to us. We could make the
model fit much better with a quadratic regression
equation,$$Y = a + b X + cX^2$$

but for our case, a simple linear regression is enough to test if the
win rate depends on deviation or not. Simplicity wins again.
...
## Summary
To summarize, we constructed two hypothesis from our dataset, and used two different tests to prove our suspicions. This was a nice exercise as this allowed us to go from theoretical usages of tests like the z-test and regression testing to using them on real data and seeing what results they bring.
