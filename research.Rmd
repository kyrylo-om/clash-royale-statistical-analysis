...
clean_data <- raw_data %>%
  rename(match_id = 1)
  #mutate(battleTime = ymd_hms(battleTime))
  #mutate(across(matches("card[0-9]+\\.id"), ~ card_map[as.character(.)])) %>%
  #mutate(
  #  winner.cards.list = str_replace_all(winner.cards.list, card_map),
  #  loser.cards.list = str_replace_all(loser.cards.list, card_map)
  #)
